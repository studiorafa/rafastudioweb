...
const Navbar = () => {
  ...
  useEffect(() => {
    // Scroll detection for navbar fade + smooth height interpolation
    const handleScroll = () => {
      setHasScrolled(window.scrollY > 50);
      
      // Schedule RAF to avoid excessive re-renders
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
      rafRef.current = requestAnimationFrame(() => {
        // Map scrollY to scale: 0px → 1.0, 100px → 0.5, 100px+ → 0.5
        const scroll = window.scrollY;
        const scale = Math.max(0.5, 1 - scroll / 200);
  ...
  const paddingTop = `${24 * heightScale}px`;
  const paddingBottom = `${24 * heightScale}px`;

  return (
    <nav 
      className={`fixed top-0 left-0 w-full z-50 flex justify-center px-6 transition-all duration-300 ${
        hasScrolled 
          ? `bg-white/95 dark:bg-black/95 backdrop-blur-md border-b ${visualTokens.borders.separator.both} shadow-sm` 
      ...
      <div className="flex items-center w-full max-w-[1200px] relative justify-between lg:justify-center">
        ...
        <div className="lg:absolute lg:left-0 lg:top-1/2 lg:-translate-y-1/2 lg:mr-16">
          <button 
            ...
            className="block opacity-100 hover:opacity-80 transition-opacity"
            aria-label="Home"
          >
            <img 
        ...
        <div className="lg:hidden flex gap-4 mr-4">
           {siteConfig.sections.map((section) => (
             <button 
                ...
                className="text-sm font-medium hover:underline underline-offset-4 transition-all text-black dark:text-white"
             >
                {/* Shorten label for mobile if needed, or use full label */}
                {section.navLabel}
             </button>
           ))}
           <button onClick={() => scrollToSection("contact")} className="text-sm font-medium hover:underline underline-offset-4 transition-all text-black dark:text-white">Contact</button>
        </div>
